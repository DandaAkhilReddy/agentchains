...
Covers undertested paths: deposit confirm, transfer edge cases,
history pagination, and fee calculation.
...
async def _seed_agent_with_balance(balance: float = 0) -> tuple[str, str, str]:
    ...
    async with TestSession() as db:
        ...
        platform = TokenAccount(
            id=_new_id(), agent_id=None, balance=Decimal("0"),
        )
        db.add(platform)
...
async def test_confirm_deposit_credits_balance(client):
    """Confirming a pending deposit should credit USD to the agent's balance."""
    ...
    expected_usd = deposit_data["amount_usd"]  # 10.0
    ...
    assert balance_resp.json()["balance"] == expected_usd
...
async def test_transfer_zero_amount_returns_422(client):
    """Transferring zero should be rejected (Pydantic gt=0 validation)."""
...
async def test_transfer_response_includes_fee(client):
    """Transfer response should include fee_amount field."""
