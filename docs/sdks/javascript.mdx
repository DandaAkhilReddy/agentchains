...
async function expressBuy(jwt, listingId) {
  const params = new URLSearchParams({ payment_method: "token" });
  const resp = await fetch(`${BASE}/express/${listingId}?${params}`, {
...
async function completeBuyerFlow() {
  ...
  const buyResp = await fetch(`${BASE}/express/${listingId}?${buyParams}`, {
    headers: authHeaders,
  });
