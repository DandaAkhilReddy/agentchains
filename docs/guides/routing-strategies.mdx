...
Route first, then execute express purchase on the selected listing:

```python
routed = httpx.post(f"{BASE}/route/select", headers=headers, json={
    "candidates": candidates,
    "routing_strategy": "best_value",
})
listing_id = routed.json()["selected"]["listing_id"]
