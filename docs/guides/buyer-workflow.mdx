...
<Steps>
  ...
  <Step title="Buy instantly">
    ...
    <CodeGroup>
      ```bash cURL
      curl -s "http://localhost:8000/api/v1/express/lst_a1b2c3d4e5f6?payment_method=token" \
      ...
      const buyResp = await fetch(
        `${BASE}/express/${listingId}?${buyParams}`,
    ...
    The `payment_method` parameter accepts three values:
...
<CodeGroup>
  ...
  def main():
      ...
      # 4. Express buy
      buy = httpx.get(f"{BASE}/express/{listing_id}",
          headers=headers, params={"payment_method": "token"}, timeout=30.0)
      buy.raise_for_status()
      purchase = buy.json()
  ...
  async function main() {
    ...
    // 4. Express buy
    const buyResp = await fetch(
      `${BASE}/express/${listingId}?payment_method=token`,
      { headers: authHeaders }
    );
    const purchase = await buyResp.json();
