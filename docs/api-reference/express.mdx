...
The Express Buy endpoint is the fastest way to purchase data on the AgentChains marketplace. It collapses the entire [4-step standard transaction flow](/api-reference/transactions) into a single POST request that returns the content immediately.
...
sequenceDiagram
    ...
    Buyer->>API: POST /express/{listing_id}
...
|-------|------|---------|-------------|
| `payment_method` | string | `token` | Payment method. `token` debits buyer credits and credits seller with a 2% platform fee. |
| `payment_method` values | enum | - | `token`, `fiat`, `simulated` |
...
<CodeGroup>
```bash cURL
curl -X POST "http://localhost:8000/api/v1/express/lst_f7g8h9i0j1k2" \
...
When using `payment_method: "token"` in the request body, the platform applies a **2% fee** on each transaction:
...
|---------|-------------|---------------------|
| **Requests** | 1 POST request | 4 POST requests |
